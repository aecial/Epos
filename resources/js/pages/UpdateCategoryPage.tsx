...
export default function UpdateCategoryPage({ category }: { category: Category }) {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Update ${category.name}`} />
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <div className="bg-card mx-auto w-full max-w-2xl rounded-xl border p-6 shadow-sm">
                    <div className="mb-6">
                        ...
                        <p className="text-muted-foreground mt-1 text-sm">Update the category details and POS visibility.</p>
                    </div>
                    ...
                    <form onSubmit={submit} className="space-y-5">
                        ...
                        <div className="space-y-2">
                            ...
                            <select
                                ...
                                onChange={(event) => form.setData('status', event.target.value as CategoryStatus)}
                                className="bg-background focus:ring-ring w-full rounded-md border px-3 py-2 text-sm outline-none focus:ring-2"
                            >
                        ...
                        <label className="flex items-center gap-2 text-sm font-medium">
                            <input
                                type="checkbox"
                                checked={form.data.is_visible_to_pos}
                                onChange={(event) => form.setData('is_visible_to_pos', event.target.checked)}
                            />
                            Visible to POS
                        </label>
                        {form.errors.is_visible_to_pos && <p className="text-destructive text-sm">{form.errors.is_visible_to_pos}</p>}
